...
export default function App() {
  ...
  const fetchDataFromApi = (latitude, longitude) => {
    if(latitude && longitude) {
      ...
      fetch(`https://api.openweathermap.org/data/2.5/weather?q=London&exclude=hourly,minutely&units=metric&lang=pt&appid=${API_KEY}`).then(res => res.json()).then(data => {
//      console.log(data)
      setData(data)
      })
    }
